...
static vprintf_like_t ovprintf = vprintf;
...
int udp_logging_vprintf( const char *str, va_list l ) {
	...
	if (strncmp(task_name, "tiT", 16) != 0)
	{
		...
		{
			#ifdef UDP_LOG_FALLBACK_MODE
				show_socket_error_reason(udp_log_fd);
				ovprintf("\nFreeing UDP Logging. sendto failed!\n", l);
				udp_logging_free(l);
				return ovprintf("UDP Logging freed!\n\n", l);
			#endif 
		}
	}
	#ifdef CONFIG_UDP_LOG_MIRROR_MODE
		return ovprintf( str, l );
	#else
		return len;
	#endif 
}

int udp_logging_init(const char *ipaddr, unsigned long port, vprintf_like_t func, vprintf_like_t fallback ) {
	if(fallback != NULL)ovprintf = fallback;
	...
	int broadcast = 1;
	err = setsockopt( udp_log_fd, SOL_SOCKET, SO_BROADCAST, &broadcast, sizeof(broadcast) );
	if (err < 0) {
