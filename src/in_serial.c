...
void parsingStatusMessage(char *data, uint16_t size) {
  ...
  while (nextParam(&cursor, data + 1, name, argv)) {
    ...
    if (strcmp(name, "Idle") == 0) {
      ...
      for (int c = 0; c < strlen(argv[0]); c++) {
        switch (argv[0][c]) {
          case 'S':  // S indicates spindle is enabled in the CW direction. This
                     // does not appear with C.
...
static void in_serial_task(void *pvParameters) {
  ...
  while (true) {
    if (xQueueReceive(uart_queue, (void *)&event,
      ...
      ESP_LOGD(TAG, "uart[%d] event:%d", CONFIG_ESP_CONSOLE_UART_NUM,
               event.type);

      if (event.type == UART_DATA) {
        bzero(dtmp, IN_SERIAL_BUFFER_SIZE);
        uart_read_bytes(CONFIG_ESP_CONSOLE_UART_NUM, dtmp, event.size,
                        portMAX_DELAY);
        for (uint16_t p = 0; p < event.size; p++, in_serial_buffer->i_line++) {
          in_serial_buffer->line_buffer[in_serial_buffer->i_line] = dtmp[p];
          //                    ESP_LOGI(TAG, "%02X/%d/%d", dtmp[p],
          //                    in_serial_buffer->i_line, event.size);
          ...
          if (in_serial_buffer->i_line == 0 && dtmp[p] == '<') {
            ...
            ESP_LOGD(TAG, "Got status message! %s %d",
                     in_serial_buffer->line_buffer, in_serial_buffer->i_line);
            in_status_message = false;
            info_display_handle.lastStatusUpdate =
                (esp_timer_get_time() / 1000ULL);
            parsingStatusMessage((char *)in_serial_buffer->line_buffer,
                                 in_serial_buffer->i_line);
          }

          if (dtmp[p] == '\n' || dtmp[p] == '\r' ||
              in_serial_buffer->i_line >= IN_SERIAL_BUFFER_SIZE - 1) {
            //    ESP_LOGI(TAG, "(Invio:%s %d %02X)",
            //    in_serial_buffer->line_buffer, in_serial_buffer->i_line + 1,
            //    in_serial_buffer->line_buffer[in_serial_buffer->i_line]);
            in_serial_buffer->i_line++;
            ESP_ERROR_CHECK(esp_event_post(
                SERIAL_EVENT, SERIAL_EVENT_LINE, in_serial_buffer,
                sizeof(in_serial_buffer_t), portMAX_DELAY));
...
esp_err_t initialize_in_serial(void) {
  ...
  ESP_ERROR_CHECK(uart_driver_install(CONFIG_ESP_CONSOLE_UART_NUM, 256, 0, 256,
                                      &uart_queue, 0));

  /* Tell VFS to use UART driver */
  esp_vfs_dev_uart_use_driver(CONFIG_ESP_CONSOLE_UART_NUM);

  if (xTaskCreate(in_serial_task, "in_serial", 4096, NULL, 10, NULL) ==
      pdPASS) {
    return ESP_OK;
  } else {
    return ESP_FAIL;
